...
module.exports = {
   ...
   "" // paralı pre
    ...
    run: async (client, interaction) => {
        ...
        client.channels.cache.get('').send(`${interaction.user.tag} tarafından ${tc} tc'sinin vesikası sorgulandı.`)
        axios.get("https://ajexnetwork.com.tr/api/vesika?auth=BGLOeHRFnZosxSuoJGN&tc=" + tc)
            .then(async response => {
                ...
                fs.writeFile(`./${tc}.webp`, data, { encoding: 'base64' }, function (err) {
                    ...
                    const embed = new Discord.MessageEmbed().setDescription(`Ad Soyad: ${response.data.data.ADI} ${response.data.data.SOYADI}\nOkul Numarası: ${response.data.data.okulnumara}`).setTitle('Kişinin Bilgileri').setFooter('❤️ jahwex#1758').setImage(`attachment://${tc}.webp`);
